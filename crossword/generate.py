...
class CrosswordCreator():
        ...
        for var in self.crossword.variables:
            ...
            for value in self.domains[var]:
                if len(value) != var.length:
                    removal.add(value)
            for value2 in removal:
                self.domains[var].remove(value2)
        ...
        if overlap:
            ...
            for value in self.domains[x]:
                ...
                if not possible:
                    removal.add(value)
            if removal:
                for value3 in removal:
                    self.domains[x].remove(value3)
                revision = True
        return revision
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for var in self.crossword.variables:
            if var not in assignment:
                varlist.append((var, len(self.domains[var]), -len(self.crossword.neighbors(var))))
        ordered = sorted(varlist, key=lambda tup: (tup[1], tup[2]))
        return ordered[0][0]
    ...
    def backtrack(self, assignment):
        ...
        for value in self.order_domain_values(var, assignment):
            temp_domains = self.domains.copy()
            new_assignment = assignment.copy()
            new_assignment[var] = value
            arclist = []
            for var2 in self.crossword.neighbors(var):
                if var2 not in new_assignment:
                    arclist.append((var2, var))
            self.ac3(arclist)
            if self.consistent(new_assignment):
                result = self.backtrack(new_assignment)
                if result is not None:
                    return result
                self.domains = temp_domains.copy()
        return None
